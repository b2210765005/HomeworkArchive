 ...
 * Extends the Game class and provides the implementation for level 1 gameplay.
 */
public class Level1 extends Game {
	 ...
	 */
	Level1(Stage primaryStage, ImageView selectedBackground, ImageView selectedCrosshair, ImageView selectedForeground, ImageView rBackground, ImageView lBackground, double width, double height) {
		StackPane level1Root = new StackPane();
		Scene level1Scene = new Scene(level1Root, width, height);
		
		selectedBackground.setFitWidth(level1Scene.getWidth());
		selectedBackground.setFitHeight(level1Scene.getHeight());
		selectedForeground.setFitWidth(level1Scene.getWidth());
		selectedForeground.setFitHeight(level1Scene.getHeight());
		selectedCrosshair.setScaleX(SCALE);
		selectedCrosshair.setScaleY(SCALE);
		
		primaryStage.setScene(level1Scene);
		Text levelText = text("LEVEL 1/6", SCALE * 7, 0, SCALE * -110);
		...
		List<String> duckPaths = Arrays.asList("assets/duck_black/4.png", "assets/duck_black/5.png", "assets/duck_black/6.png");
		...
		final double[] move = {SCALE * 10};
		duck.setTranslateY(-SCALE * 35);
		...
		duckAnimation.getKeyFrames().add(new KeyFrame(Duration.millis(200), e -> {
			...
			double newPositionX = duck.getTranslateX() + move[0];
			duck.setImage(ducks.get(nextIndex));
			
			if (newPositionX >= selectedBackground.getTranslateX() + width / 2 && move[0] > 0 || newPositionX <= selectedBackground.getTranslateX() - width / 2 && move[0] < 0) {
		...
		level1Scene.setOnMouseClicked(e -> {
			if (mermi != 0) {
				...
				if (distance <= SCALE * 15) {
					...
					mediaPlayer2.play();
					level1Scene.setOnKeyPressed(e1 -> {
						if (e1.getCode() == KeyCode.ENTER) {
							mediaPlayerr.stop();
							mediaPlayer2.stop();
							new Level2(primaryStage, selectedBackground, selectedCrosshair, selectedForeground,rBackground,lBackground, width, height);
						}
					});
		...
		level1Scene.setCursor(Cursor.NONE);
		level1Scene.setOnMouseMoved(e -> {
		...
		level1Root.getChildren().addAll(selectedBackground, rBackground, lBackground, duck, selectedForeground, gameText, levelText, ammoText, text1, text2, selectedCrosshair);
